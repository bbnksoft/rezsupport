...
const CabinManagement: React.FC<CabinManagementProps> = ({ ship, deck, onPageChange }) => {
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');

  const cabins: Cabin[] = [
    { id: '11001', type: 'Oceanview', status: 'available', price: 899 },
    { id: '11002', type: 'Oceanview', status: 'available', price: 899 },
    { id: '11003', type: 'Oceanview', status: 'booked', price: 899, guest: 'John Smith' },
        ...
        <div className="card">
          <div className="card-body">
            ...
            <div style={{
              ...
              {cabins.map(cabin => (
                <div
                  ...
                  style={{
                    background: 
                      cabin.status === 'available' ? '#F0FDF4' :
                      cabin.status === 'booked' ? '#EFF6FF' :
                      cabin.status === 'vip' ? '#FEFCE8' :
                      '#FEF2F2',
                    borderRadius: '8px',
                    padding: '15px',
                    textAlign: 'center',
                    ...
                    transition: 'all 0.2s',
                    border: 
                      cabin.status === 'available' ? '2px solid #BBF7D0' :
                      cabin.status === 'booked' ? '2px solid #BFDBFE' :
                      cabin.status === 'vip' ? '2px solid #FDE68A' :
                      '2px solid #FECACA',
                    opacity: cabin.status === 'maintenance' ? 0.75 : 1
                  }}
                >
                  <div style={{ fontSize: '18px', fontWeight: 700, marginBottom: '8px', color: '#0F172A' }}>
                    {cabin.id}
                  </div>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 600, color: '#475569' }}>
                    {cabin.status === 'booked' ? 'BOOKED' :
                      cabin.status === 'maintenance' ? 'MAINT' :
                      `$${cabin.price}`}
                  </div>
                  {cabin.guest && (
                    <div style={{ fontSize: '11px', color: '#7E8299', marginTop: '4px' }}>
                      {cabin.guest}
                    </div>
                  )}
                </div>
            ...
            <div style={{ marginTop: '30px', padding: '15px', background: '#F9F9F9', borderRadius: '8px' }}>
              <div style={{ display: 'flex', gap: '20px', fontSize: '13px', justifyContent: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: '12px', height: '12px', background: '#50CD89', borderRadius: '2px' }}></div>
                  <span>Available</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: '12px', height: '12px', background: '#3699FF', borderRadius: '2px' }}></div>
                  <span>Booked</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: '12px', height: '12px', background: '#FFC700', borderRadius: '2px' }}></div>
                  <span>VIP</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: '12px', height: '12px', background: '#F1416C', borderRadius: '2px' }}></div>
                    ...
                    <tr key={cabin.id}>
                      ...
                      <td>
                        <span className={`badge badge-${
                          cabin.status === 'available' ? 'success' :
                          cabin.status === 'booked' ? 'primary' :
                          cabin.status === 'vip' ? 'warning' :
                          'danger'
                        }`}>
                          {cabin.status}
                        </span>
                      ...
                      <td>
                        <div className="action-buttons">
                          <button className="btn btn-primary btn-sm">👁️ View</button>
